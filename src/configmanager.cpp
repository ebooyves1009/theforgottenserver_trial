...
bool ConfigManager::load()
{
	...
	if (!loaded) { // info that must be loaded one time (unless we reset the modules involved)
		...
		string[MYSQL_USER] = getGlobalString(L, "mysqlUser", getEnv("MYSQL_USER", "root"));
		string[MYSQL_PASS] = getGlobalString(L, "mysqlPass", getEnv("MYSQL_PASSWORD", "1236"));
		string[MYSQL_DB] = getGlobalString(L, "mysqlDatabase", getEnv("MYSQL_DATABASE", "testdatabase"));
